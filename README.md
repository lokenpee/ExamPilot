# ExamPilot · 资料驱动的 AI 组卷与自测 Agent

本地运行的资料驱动 AI 组卷与自测 Demo。上传课程、培训或产品资料，由规划 Agent 安排试卷，出题 Agent 通过真实 MCP 批量取得证据，再逐题生成，并导出学生版试卷与教师版答案解析。既适合教师和培训讲师出题，也适合学生自测、产品/销售经理做培训考核。删除题目后才能对话补题；候选题可多轮调整，采纳后才入卷。

## 适用场景

- **教师/培训讲师**：课程单元测试、期末考试、企业内训测验。
- **学生/学习者**：基于教材、课件自测、复习和错题巩固。
- **产品/销售经理**：把产品手册、销售话术、合规资料转成培训考核卷。
- **企业培训/L&D**：将内部资料快速生成试卷、标准答案和解析。
...
8. 总分或来源硬错误必须修复；内容提醒可由出题人/教师核对后确认。导出 Word 学生版与教师答案解析版。

出题人删除题目后目标总分不自动变化。可以补题恢复原总分，或通过试卷标题区的总分编辑入口修改目标，再导出。
...
**自动化工作流测试中的模型响应使用 `tests/test_workflow.py` 内的确定性替身；应用和 stdio MCP 使用真实实现。另已使用真实 `deepseek-flash` 完成 27 块资料分析、两轮规划、数据库原理 13 题／100 分生成与补题，以及财政学 38 题／200 分真实规划、生成与双 Word 导出；自动回归 25 项通过。见 [真实联调记录](docs/demo/deepseek-flash-live.md)。GPT、硅基流动及其他具体模型仍需各自验收。**
...
- DeepSeek Flash 已完成数据库原理与财政学两份真实样例的业务闭环；不同学科和三家供应商的其他模型仍需分别验证，不能将这些样例视为全部出题质量保证。
- 扫描件、图片文字、复杂公式与图表不做 OCR。纯图片页会提示转换资料；混合页面只处理可提取文字，视觉内容需出题人/教师另行核对。
...
产品规则见 [ExamPilot-PRD.md](ExamPilot-PRD.md)，实现与验收记录见 [docs/demo/implementation-status.md](docs/demo/implementation-status.md)。
